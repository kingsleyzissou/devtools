...
import sys
import dotenv
from devtools import echo, load_json, write_yaml, sanitize_output
...
class BaseImage:
    ...
    def get_image_location(self, location) -> tuple:
        ...
        try:
            self.guest_image = os.path.join(
                SCRATCH_DIR, images[self.arch][self.distro])
            image_basename = os.path.splitext(
                os.path.basename(self.guest_image))[0]
            self.overlay_image = os.path.join(
                OVERLAY_DIR, f"{image_basename}.overlay.qcow2")
        except Exception:
    ...
    def get_repos(self) -> None:
        ...
        for r in repos["yum_repos"]:
            repos["yum_repos"][r]["baseurl"] = repos["yum_repos"][r]["baseurl"].replace(
                "${arch}", self.arch)
    ...
    def make_overlay(self) -> None:
        echo("Making overlay")
        subprocess.run(["qemu-img", "create", "-q", "-o",
                       f"backing_file={self.guest_image},backing_fmt=qcow2", "-f", "qcow2", self.overlay_image])
        subprocess.run(["qemu-img", "resize", "-q",
                       self.overlay_image, "100G"])
    ...
    def make_metadata(self) -> None:
        ...
        meta_data["local-hostname"] = meta_data["local-hostname"].replace(
            "${image_name}", self.distro)
